...
class Receiver():
        ...
        while True:
            try:
                ...
                if Checksum.validate_checksum(message):
                    # 检查序列号是否是重包                    
                    if any(x[0][1] == seqno for x in buffer) or seqno < need_seqno:
                        #print"find_same_seqno %s" % seqno
                        self._send_ack(need_seqno, address)#ack
                        #for item in buffer:
                        self._send_buffer(seqno,address)
                        #pass
                    else:
                        ...
                        while (int(need_seqno) == int(buffer[0][1])):
                            ...
                            if not buffer:
                                break
                        #for item in buffer:
                            #self._send_buffer(item[1],address)
                        self._send_ack(need_seqno, address)#ack
    ...
    def _send_buffer(self, seqno, address):
        m = "buffer|%d|" % seqno
